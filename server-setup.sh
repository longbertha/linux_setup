...
# 安装常用基础组件
install_components() {
	echo "此操作将安装: fail2ban, vim, chrony, curl, rsync, wget, jq"
...
configure_fail2ban() {
	...
	local script_url="https://raw.githubusercontent.com/longbertha/linux_setup/main/configure_fail2ban.sh"
...
set_dns_dhclient() {
	...
	bash <(wget -qO - "https://raw.githubusercontent.com/longbertha/linux_setup/main/set_dns_via_dhclient.sh")
...
display_menu() {
	...
	clear
	echo -e "${BOLD}欢迎使用 Linux 工具箱${RESET}"
	echo -e "${BOLD}GitHub：https://github.com/longbertha/linux_setup${RESET}"
	echo -e "-----------------------------------------------------"
	...
	echo -e "${GREEN} 1${RESET}       安装常用组件 (Fail2ban...)"
